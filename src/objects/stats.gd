...
@onready var score_label: Label = $base/score
@onready var flair: Label = $base/flair
@onready var level_label: Label = $base/level

func connect_signals():
    PROVIDER.connect("score_flair", score_flair)
    return
...
func _process(delta: float) -> void:
    update_score()
    level_label.text = str(PROVIDER.current_level)
    pass
