...
func connect_signals():
    PROVIDER.connect("generate_tiles", generate_tiles)
    return
...
func _process(delta: float):
    if PROVIDER.tiles_clickable:
        handle_flipped_tiles()
    if get_child_count() <= 0:
        PROVIDER.emit_signal('level_complete')
    return
...
func update_score(amount: int) -> void:
    ...
    PROVIDER.emit_signal("score_flair", delta_score)
